...
from app.schemas.meta import DepartmentResponse, TeamResponse, ProjectMetaResponse, UserMetaResponse
...
def get_projects(
    ...
    user_project_ids = [
        pm.project_id for pm in db.query(ProjectMember).filter(ProjectMember.user_id == current_user.id).all()
    ]
...
def get_users(
    ...
    if project_id is not None:
        ...
        query = db.query(User).join(ProjectMember, User.id == ProjectMember.user_id).filter(
